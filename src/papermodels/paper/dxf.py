...
def load_dxf_annotations(
        ...
        dxf_dir: Optional[pathlib.Path] = None,
        annotations_layer: Optional[str] = None,
        pages_layer: Optional[str] = None,
        show_skipped: bool = False) -> list[Annotation]:
    """
    ...
    msp = doc.modelspace()
    pages_layer = msp.query("layer==pages")
    annotations_layer = msp.query('layer==annotations')
    for annotation in list(annotations_layer):
        annots_in_dxf = []
        skipped_annots = []
...
def update_pdf_annotations(
    ...
    """
    Returns None. Creates a copy of the file at 'dxf_path' with all of annotations representing 
    ...
    """
    dxf_path = pathlib.Path(dxf_path).resolve()

    with pike.open(dxf_path) as pdf_obj:
